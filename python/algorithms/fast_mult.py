...
# to assist in addition make it O(n) instead
# of inverting array to int back to array (which is still O(n))
# carry adder implementation
def bin_list_addition(x, y):
    longer = x
    shorter = y
    if len(y) > len(x):
        longer = y
...
def reference_multiply(x, y):
    # assuming that the binary numbers are an array
    # x_int = bin_list_to_int(x)
    # y_int = bin_list_to_int(y)
